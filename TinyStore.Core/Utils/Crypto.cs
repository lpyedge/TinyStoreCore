...
namespace TinyStore.Utils
{
    ...
    public static class HASHCrypto
    {
        ...
        public static HashAlgorithm Generate(CryptoEnum p_CryptoEnum = CryptoEnum.SHA1, string p_SecretKey = "")
        {
            ...
            return Generate2(p_CryptoEnum, secretBuff);
        }
        ...
        /// <returns>返回加密类HashAlgorithm</returns>
        public static HashAlgorithm Generate2(CryptoEnum p_CryptoEnum = CryptoEnum.SHA1, byte[] p_SecretBuff = null)
        {
    ...
    public static class DESCrypto
    {
        ...
        public static SymmetricAlgorithm Generate(string p_KeyIV,
            ...
            CipherMode p_CipherMode = CipherMode.CBC, PaddingMode p_PaddingMode = PaddingMode.PKCS7, int p_BlockSize = 0, int p_KeySize = 0)
        {
            ...
            CipherMode p_CipherMode = CipherMode.CBC, PaddingMode p_PaddingMode = PaddingMode.PKCS7, int p_BlockSize = 0, int p_KeySize = 0)
        {
            ...
            switch (p_CryptoEnum)
            {
                case CryptoEnum.AES:
                    {
                        if (!string.IsNullOrWhiteSpace(p_Key) && (Encoding.UTF8.GetByteCount(p_Key) == 16 || Encoding.UTF8.GetByteCount(p_Key) == 24 || Encoding.UTF8.GetByteCount(p_Key) == 32))
                        {
                ...
                case CryptoEnum.DES:
                    {
                        if (!string.IsNullOrWhiteSpace(p_Key) && (Encoding.UTF8.GetByteCount(p_Key) == 16 || Encoding.UTF8.GetByteCount(p_Key) == 24))
                        {
                            provider.Key = Encoding.UTF8.GetBytes(p_Key);
                        }
                        if (p_CipherMode != CipherMode.ECB)
                ...
                case CryptoEnum.RC2:
                    {
                        if (!string.IsNullOrWhiteSpace(p_Key) && (Encoding.UTF8.GetByteCount(p_Key) >= 5 && Encoding.UTF8.GetByteCount(p_Key) <= 16))
                        {
                            provider.Key = Encoding.UTF8.GetBytes(p_Key);
                        }
                        if (p_CipherMode != CipherMode.ECB)
    ...
    public static class RSACrypto
    {
        private class Extensions
        {
            ...
            static public byte[] _SeqOID = new byte[] { 0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01, 0x01, 0x05, 0x00 };
        ...
        /// <returns></returns>
        public static byte[] SignData(this RSACryptoServiceProvider p_Provider, HASHCrypto.CryptoEnum p_Hashenum, byte[] p_InputBuff)
        {
            if (p_InputBuff != null)
            {
        ...
        /// <param name="p_Hashenum">签名类型</param>
        /// <returns></returns>
        public static bool VerifyData(this RSACryptoServiceProvider p_Provider, HASHCrypto.CryptoEnum p_Hashenum, byte[] p_SignedBuff, byte[] p_InputBuff)
        {
            if (p_InputBuff != null)
            {
                HashAlgorithm hashalg = null;
