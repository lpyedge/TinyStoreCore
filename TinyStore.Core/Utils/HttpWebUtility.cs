...
namespace TinyStore.Utils
{
    public class HttpWebUtility : IDisposable
    {
            ...
            Encoding p_encoding = null, string p_boundary = null)
        {
            ...
            return p_encoding.GetString(ResponseBodyBinary(p_url, p_httpMethod, p_encoding.GetBytes(p_queryData),
            ...
            if (myRequest != null)
            {
                ...
                try
                {
                    using (var myResponse = myRequest.GetResponse() as HttpWebResponse)
                    {
                        ...
                        // {
                        //     case "gzip":
                        //         using (var myGzipStream = new GZipStream(myStream, CompressionMode.Decompress))
                        //         {
                        //             do
                        //             {
                        //                 bytenow = myGzipStream.ReadByte();
                        //                 if (bytenow != -1)
                        //                     buffer.Add(byte.Parse(bytenow.ToString(CultureInfo.InvariantCulture)));
                        //             } while (bytenow != -1);
                        //         }
                        //
                        //         break;
                        //     case "deflate":
                        //         using (var myDeflateStream = new DeflateStream(myStream, CompressionMode.Decompress))
                        //         {
                        //             do
                        //             {
                ...
                catch (WebException webEx)
                {
                    ...
                    {
                        // var myStream = webEx.Response.GetResponseStream();
                        // var buffer = new List<byte>();
                        // int bytenow;
                        // switch ((webEx.Response as HttpWebResponse).ContentEncoding?.ToLower())
                        // {
                        //     case "gzip":
                        //         using (var myGzipStream = new GZipStream(myStream, CompressionMode.Decompress))
                        //         {
        ...
        {
            return MyCookieContainer.GetCookies(p_url).Cast<Cookie>().ToDictionary(item => item.Name);
        }
