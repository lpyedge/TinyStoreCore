...
namespace Payments
{
    public abstract class IPayChannel
    {
        ...
        {
            PayChannnelAttr = GetType().TypeChannelAttribute();
        ...
        public PlatformAttribute PlatformAttr => Utils.Core.EnumAttribute<PlatformAttribute>(this.Platform);

        public PayChannelAttribute PayChannnelAttr { get; protected set; }


        /// <summary>
        ///     支付接口名称
        /// </summary>
        public virtual string Name => this.Platform.ToString() 
                                      + "_"  + PayChannnelAttr.Channel.ToString() 
                                      + "_" + PayChannnelAttr.PayType.ToString();

        /// <summary>
        ///     支付接口注释
        /// </summary>
        public virtual string Memo => PlatformAttr.Name 
                                      + "_" + PayChannnelAttr.ChannelDesc 
                                      + "_" + PayChannnelAttr.PayTypeDesc;
        ...
        public virtual string SettingsJson
        {
            ...
            set
            {
                ...
                if (!string.IsNullOrWhiteSpace(value))
                    foreach (SettingBase s in Utils.JsonUtility.Deserialize<List<SettingBase>>(value))
                    {
