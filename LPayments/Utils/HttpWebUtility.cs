...
public sealed class HttpWebUtility : IDisposable
{
        ...
        Dictionary<string, dynamic> paramList = null!, Dictionary<string, string> headList = null!)
    {
               ...
               new MemoryStream(await ResponseBinaryAsync(url, httpMethod, paramList!, headList!)))
        {
            ...
            {
                resultStr = await streamReader.ReadToEndAsync();
            }
        }
        ...
        using (var myRequest = new HttpRequestMessage(new System.Net.Http.HttpMethod(httpMethod.ToString()), url))
        {
            ...
            if (httpMethod is HttpMethod.POST or HttpMethod.PUT or HttpMethod.PATCH or HttpMethod.DELETE)
            {
                ...
                else
                {
                    ...
                    myRequest.Content.Headers.ContentType =
                        new MediaTypeHeaderValue(contentTypes.FirstOrDefault(p => !p.Contains("="))?.Trim());
                    for (int i = 1; i < contentTypes.Length; i++)
                    {
                        myRequest.Content.Headers.ContentType.Parameters.Add(
                            contentTypes[i].Contains("=")
                                ? new NameValueHeaderValue(contentTypes[i].Split('=')[0].Trim(),
                                    contentTypes[i].Split('=')[1].Trim())
    ...
    {
        return await MyHttpClient.SendAsync(httpRequestMessage, HttpCompletionOption.ResponseHeadersRead);
    }

    public async Task<HttpResponseMessage> PostAsync(Uri requestUri, HttpContent content,
        HttpRequestHeaders headers = null!)
    {
        var requetMessage = new HttpRequestMessage(System.Net.Http.HttpMethod.Post, requestUri);
        requetMessage.Content = content;
        if (headers != null && headers.Any())
        {
            foreach (var item in headers)
            {
                requetMessage.Headers.Append(item);
            }
        }

        return await SendSync(requetMessage);
    }

    public async Task<string> PostStringAsync(Uri requestUri, string content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new StringContent(content), headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsStringAsync();
    }

    public async Task<string> PostStringAsync(Uri requestUri, byte[] content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new ByteArrayContent(content), headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsStringAsync();
    }

    public async Task<byte[]> PostByteArrayAsync(Uri requestUri, string content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new StringContent(content), headers!);
        response.EnsureSuccessStatusCode();
    ...
    public async Task<byte[]> PostByteArrayAsync(Uri requestUri, byte[] content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new ByteArrayContent(content), headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsByteArrayAsync();
    ...
    public async Task<Stream> PostStreamAsync(Uri requestUri, string content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new StringContent(content), headers!);
        response.EnsureSuccessStatusCode();
    ...
    public async Task<Stream> PostStreamAsync(Uri requestUri, byte[] content, HttpRequestHeaders headers = null!)
    {
        var response = await PostAsync(requestUri, new ByteArrayContent(content), headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsStreamAsync();
    ...
    public async Task<HttpResponseMessage> GetAsync(Uri requestUri, HttpRequestHeaders headers = null!)
    {
        var requetMessage = new HttpRequestMessage(System.Net.Http.HttpMethod.Get, requestUri);
        if (headers != null && headers.Any())
        {
            foreach (var item in headers)
            {
    ...
    public async Task<Stream> GetStreamAsync(Uri requestUri, HttpRequestHeaders headers = null!)
    {
    ...
    public async Task<byte[]> GetByteArrayAsync(Uri requestUri, HttpRequestHeaders headers = null!)
    {
        var response = await GetAsync(requestUri, headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsByteArrayAsync();
    ...
    public async Task<string> GetStringAsync(Uri requestUri, HttpRequestHeaders headers = null!)
    {
        var response = await GetAsync(requestUri, headers!);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsStringAsync();
    ...
    public HttpClientHandler MyHttpClientHandler { get; private set; }
    public Uri CurrentUri { get; private set; }
    public Encoding CurrentCharset { get; private set; } = Encoding.UTF8;
    public Dictionary<string, IEnumerable<string>> CurrentHeaders { get; private set; }
    ...
    public AuthenticationHeaderValue AuthenticationHeader { get; private set; } = null;
    ...
    {
        return MyHttpClientHandler.CookieContainer.GetCookies(url).Cast<Cookie>()
    ...
    {
        var certificate = new X509Certificate2(string.IsNullOrEmpty(certificatePassword)
    ...
    private readonly Func<Stream, string, byte[]> ReadCompressionStream = (inputStream, compression) =>
    {
        if (inputStream != null)
        {
            Stream stream;
    ...
    private static string GetBoundary => $"-----{Convert.ToInt64(DateTime.Now.Ticks):x}-----";
